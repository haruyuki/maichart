...
export const GridItem: React.FC<GridItemProps> = ({ song, index, chartType, coverArtUrl }) => {
  // Padding and sizing
  const paddingX = 8;
  const paddingY = 0;
  ...
  if (isPlaceholder) {
    // Render maimai-style placeholder for NO DATA
    return (
      <div
      ...
      >
        <div className="text-center">
          ...
          <div className="maimai-text-outline text-lg" style={{ fontSize: '16px' }}>
            NO DATA
          </div>
        </div>
      </div>
  ...
  return (
    <div
      className="relative overflow-hidden rounded-md border border-white/40 shadow-lg"
      style={{
        width: 200,
        height: 150,
    ...
    >
      {/* Cover art as a background, with blurred and dark overlay */}
      {coverArtUrl && (
        <>
          <div
      ...
      {/* Difficulty triangle */}
      <svg className="absolute top-0 right-0 z-20" width={60} height={60}>
        <polygon points="60,0 22,0 60,38" fill={difficultyColor} fillOpacity={0.85} />
      </svg>
      {/* Content */}
      <div
      ...
      >
        ...
        <div
          ...
          style={{ padding: `0 ${paddingX}px ${paddingY}px ${paddingX}px`, position: 'relative' }}
        >
          <div>
            <div style={{ fontWeight: 100, fontSize: FONT_SIZE_ACHIEVEMENT }}>
              {song.achievement.toFixed(4)}
            </div>
            <div style={{ fontWeight: 500, fontSize: FONT_SIZE_ACHIEVEMENT_RATING, lineHeight: 1 }}>
              {getAchievementRating(song.achievement)}
            </div>
          </div>
          <div
            className="text-right"
            style={{ position: 'absolute', right: paddingX, bottom: paddingY, lineHeight: 1 }}
          >
            <div style={{ fontWeight: 500, fontSize: FONT_SIZE_LEVEL }}>
              {song.level !== undefined ? song.level : '-'}
            </div>
